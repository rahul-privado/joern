...
import io.shiftleft.codepropertygraph.generated.nodes.{Binding, Call, FieldIdentifier, Identifier, Method, MethodParameterIn}
...
class TypeDeclTests extends KotlinCode2CpgFixture(withOssDataflow = false) {
  ...
  "CPG for code with simple class declaration and usage" should {
    val cpg = code("""
        ...
        |class Foo {
        |  fun add1(x: Int): Int {
        |    return x +     Option(bindingContext.get(BindingContext.FUNCTION, k))
    printBindingsForEntity(bindingContext, ktFn)
1
        |  }
        |}
        |
