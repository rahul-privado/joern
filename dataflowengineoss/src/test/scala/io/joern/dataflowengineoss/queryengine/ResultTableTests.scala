...
class ResultTableTests extends AnyWordSpec with Matchers {
  ...
  "ResultTable::add" should {
    ...
    "append to existing results on multiple inserts for same key" in {
      ...
      val res1  = Vector(ReachableByResult(TaskFingerprint(node1), Vector(PathElement(node1)), List()))
      val res2  = Vector(ReachableByResult(TaskFingerprint(node1), Vector(PathElement(node2)), List()))
      table.add(TaskFingerprint(node1), res1)
      table.add(TaskFingerprint(node1), res2)
      table.get(TaskFingerprint(node1)) match {
        case Some(results) =>
  ...
  "ResultTable::createFromTable" should {
    ...
    "correctly combine path and path stored in table on createFromTable" in {
      ...
      val table               = new ResultTable
      table.add(
        TaskFingerprint(pivotNode),
        Vector(ReachableByResult(TaskFingerprint(node1), pathContainingPivot, List()))
      )
